...
class HrEmployee(models.Model):
    ...
    shift_type = fields.Many2one('hr.shift', string='Shift')
    next_shift_type = fields.Many2one('hr.shift', string='Shift Minggu Depan')
    is_scheduled = fields.Boolean(string='Scheduled Employee')
    ...
    weekly_hours = fields.Float(string="Weekly Hours", readonly=True)
...
@api.model
def cron_update_weekly_shift(self):
        """Cron otomatis setiap hari Minggu untuk update shift minggu depan"""
        employees = self.search([('is_scheduled', '=', True)])
        mail_template = self.env.ref('hr_employee_schedule_plus.email_template_shift_update', False)

        for emp in employees:
            if not emp.next_shift_type:
                continue  # jika tidak ada shift minggu depan, skip

            # update shift
            emp.shift_type = emp.next_shift_type

            # reset next shift
            emp.next_shift_type = False

            # kirim email notifikasi
            if mail_template:
                mail_template.send_mail(emp.id, force_send=True)
